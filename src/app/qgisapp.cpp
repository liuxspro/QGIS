...
void QgisApp::updateRecentProjectPaths()
{
  ...
  {
    QgsProjectStorage *storage = QgsApplication::projectStorageRegistry()->projectStorageFromUri(
      recentProject.path
    );
    QString storageFilePath;
    QString displayPath = recentProject.path;
    if ( storage )
      storageFilePath = storage->filePath( recentProject.path );
    QString displayPath = !storageFilePath.isEmpty() ? storageFilePath : recentProject.path;
